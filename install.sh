...
detect_os() {
    ...
    case "$OS" in
        ...
        almalinux|rocky) PACKAGE_MANAGER="dnf" ;;
...
install_base_packages() {
    ...
    case "$PACKAGE_MANAGER" in
        apt)
            apt install -y git curl wget unzip zip tar ca-certificates software-properties-common
        ;;
        dnf)
            dnf install -y git curl wget unzip zip tar
        ;;
        yum)
            yum install -y git curl wget unzip zip tar
        ;;
    esac
...
install_php() {
    ...
    case "$PACKAGE_MANAGER" in
        apt)
            ...
            apt install -y php8.3 php8.3-cli php8.3-fpm php8.3-common php8.3-mysql php8.3-curl php8.3-mbstring php8.3-xml php8.3-bcmath php8.3-gd php8.3-intl php8.3-zip
            PHP_BIN="/usr/bin/php"
        ...
        dnf)
            ...
            dnf install -y php php-cli php-fpm php-common php-mysqlnd php-gd php-curl php-intl php-xml php-bcmath php-mbstring php-zip
            PHP_BIN="/usr/bin/php"
        ...
        yum)
            ...
            yum install -y php php-cli php-fpm php-common php-mysqlnd php-gd php-curl php-intl php-xml php-bcmath php-mbstring php-zip
            PHP_BIN="/usr/bin/php"
            PHP_FPM_SERVICE="php-fpm"
        ;;
    esac
...
install_mariadb() {
    ...
    case "$PACKAGE_MANAGER" in
        apt) apt install -y mariadb-server ;;
        dnf) dnf install -y mariadb-server ;;
        yum) yum install -y mariadb-server ;;
    esac
...
###############################################################################
# NEW: Detect incomplete installation
###############################################################################

detect_incomplete_install() {
    if [ -f "$INSTALL_DIR/install.json" ] && [ ! -f "$INSTALL_DIR/version" ]; then
        echo
        echo "Previous incomplete installation detected."
        echo
...
generate_app_key() { APP_KEY=$(openssl rand -hex 32); }
...
create_superadmin() {
    ...
    php cli/install.php --username="$ADMIN_USER" --password="$ADMIN_PASS"
...
if [ $# -eq 0 ]; then
    ...
    case "$opt" in
        1) bash "$BASE/update.sh" ;;
        2) bash "$BASE/backup.sh" ;;
        3) bash "$BASE/restore.sh" ;;
        4) bash "$BASE/change-port.sh" ;;
        5)
            systemctl restart nginx
            systemctl restart mariadb
        ...
        ;;
        6) bash "$BASE/repair.sh" ;;
        7) systemctl status nginx --no-pager ;;
        8) cat "$BASE/version" ;;
        9) bash "$BASE/uninstall.sh" ;;
        *) exit 0 ;;
    esac
...
case "$1" in
    update) bash "$BASE/update.sh" ;;
    backup) bash "$BASE/backup.sh" ;;
    restore) bash "$BASE/restore.sh" ;;
    repair) bash "$BASE/repair.sh" ;;
